void SerialScan (void *p) {
  ...
  while (1) {
    ...
    // ============================================================
    // ==  Accepter un nouveau client TCP WiFi (XCSoar via WiFi) ==
    // ============================================================
    if (!nmeaClient || !nmeaClient.connected()) {
      ...
      if (newClient) {
        ...
        SourceIsWifi = true;
        Serial.println("XCSoar TCP connecte depuis : " + nmeaClient.remoteIP().toString());
      }
    }

    // ============================================================
    // ==  Lecture prioritaire depuis WiFi TCP si connecte        ==
    // ============================================================
    if (SourceIsWifi && nmeaClient && nmeaClient.connected() && nmeaClient.available()) {
      ...
      if (serialString == '$') {
        ...
        if (timeSystemReady < 2000) {
          // Envoyer les parametres initiaux via TCP au lieu de Serial2
    ...
    // ============================================================
    // ==  Lecture depuis Serial2 si pas de connexion WiFi       ==
    // ============================================================
    else if (!SourceIsWifi && Serial2.available()) {
    ...
    if (dataString.startsWith("$PFV") && SourceIsXCSoar == true) {
      if (serial2Error == true) {
        ...
        String syncQnh = (qnhStr + String(checksumQnh, HEX));
        if (SourceIsWifi && nmeaClient.connected()) nmeaClient.println(syncQnh);
        else Serial2.println(syncQnh);

        // synchronize Bug after reboot of OV
        String bugStr = ("$PFV,B,S," + String(valueBugAsFloat) + "*");
        int checksumBug = calculateChecksum(bugStr);
        String syncBug = (bugStr + String(checksumBug, HEX));
        if (SourceIsWifi && nmeaClient.connected()) nmeaClient.println(syncBug);
        else Serial2.println(syncBug);

        // synchronize MC after reboot of OV
        String mcStr = ("$PFV,M,S," + valueMacAsString + "*");
        int checksumMc = calculateChecksum(mcStr);
        String synMc = (mcStr + String(checksumMc, HEX));
        if (SourceIsWifi && nmeaClient.connected()) nmeaClient.println(synMc);
        else Serial2.println(synMc);
      }
